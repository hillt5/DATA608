...
preg_bc <- left_join(x = pregnancy_fix, y = bc_same_hosp, by = c('Facility.ID' = 'ID')) #Join new hospital info 
preg_bc <- preg_bc %>%
  drop_na()
...
county_bd_births_0914 <- cbind(bd_births_county, county_pop_0914[,2]) %>%
